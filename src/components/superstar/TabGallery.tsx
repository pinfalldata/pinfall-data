...
export default function TabGallery({ superstar }: { superstar: any }) {
  ...
  const resetFilters = () => {
    const next = { year: '', mediaType: '', showSeriesId: '' }
    setFilters(next)
    fetchGallery(1, next)
  }
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        <div className="flex items-center rounded-xl border border-border-subtle/30 overflow-hidden">
          {[
            ...
            <button
              key={opt.key}
              onClick={() => applyFilter('mediaType', opt.key)}
              className={`px-3 py-2 text-xs font-medium transition-all ${
                filters.mediaType === opt.key
                  ? 'bg-neon-blue/15 text-neon-blue'
                  : 'text-text-secondary hover:text-text-white hover:bg-bg-secondary/30'
              }`}
            >{opt.label}</button>
          ))}
        </div>
        ...
        {filterOpts.showSeries.length > 0 && (
          <select
            ...
            onChange={e => applyFilter('showSeriesId', e.target.value)}
            className="px-3 py-2 rounded-xl bg-bg-secondary/30 border border-border-subtle/30 text-xs text-text-white appearance-none cursor-pointer focus:outline-none focus:border-neon-blue/40"
          >
        ...
        {/* Reset */}
        {hasActiveFilters && (
          <button onClick={resetFilters} className="px-3 py-2 rounded-xl text-xs text-red-400 hover:bg-red-400/10 transition-colors">
            Clear filters
          </button>
        )}
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="aspect-video rounded-xl bg-bg-secondary/30 animate-pulse" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-text-secondary">
            {hasActiveFilters ? 'No media found with these filters.' : 'No media available yet.'}
          </p>
          {hasActiveFilters && (
            <button onClick={resetFilters} className="mt-3 text-xs text-neon-blue hover:text-neon-blue/80 transition-colors">
              Clear all filters
            </button>
          )}
        </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {getVisiblePages(page, totalPages).map((p, i) =>
              ...
              <button key={p} onClick={() => goPage(p as number)}
                className={`w-8 h-8 rounded-lg text-xs font-medium transition-all ${p === page ? 'bg-neon-blue/20 border border-neon-blue/40 text-neon-blue' : 'border border-transparent text-text-secondary hover:text-text-white hover:bg-bg-secondary/50'}`}
              >{p}</button>
            )}
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4" onClick={() => setLightbox(null)}>
          ...
          {(() => {
            ...
            return <>
              {idx > 0 && (
                <button onClick={(e) => { e.stopPropagation(); setLightbox(items[idx - 1]) }}
                  className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-[110] w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
              )}
              {idx < items.length - 1 && (
                <button onClick={(e) => { e.stopPropagation(); setLightbox(items[idx + 1]) }}
                  className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-[110] w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors">
          ...
          <div className="max-w-5xl w-full flex flex-col items-center" onClick={e => e.stopPropagation()}>
                ...
                if (ytId) return (
                  <div className="w-full rounded-2xl overflow-hidden bg-black">
                    <div className="relative w-full pt-[56.25%]">
                      <iframe src={`https://www.youtube.com/embed/${ytId}?autoplay=1`} title={lightbox.title || ''} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="absolute inset-0 w-full h-full" />
                    </div>
                  </div>
                )
            ...
            <div className="mt-4 text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-1 flex-wrap">
                {lightbox.show && (
                  <Link href={`/shows/${lightbox.show.slug}`} onClick={() => setLightbox(null)} className="text-xs text-neon-blue hover:text-neon-blue/80 transition-colors">
                    {lightbox.show.name}
                  </Link>
                )}
...
function GalleryCard({ item, onClick }: { item: MediaItem; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="group relative aspect-video rounded-xl overflow-hidden bg-bg-tertiary/30 border border-border-subtle/15 hover:border-neon-blue/25 transition-all hover:shadow-[0_0_20px_rgba(199,160,90,0.06)] text-left">
      {thumb ? (
        <Image src={thumb} alt={item.title || ''} fill sizes="(max-width:640px) 50vw,(max-width:1024px) 33vw,16vw" className="object-cover group-hover:scale-105 transition-transform duration-300" />
      ) : (
      ...
      <div className="absolute top-1.5 left-1.5">
        <span className={`text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${
          isVideo ? 'bg-red-600/90 text-white' : 'bg-neon-blue/80 text-white'
        }`}>{isVideo ? 'Video' : 'Photo'}</span>
      </div>
