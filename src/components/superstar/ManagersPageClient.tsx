    ...
    <div className="min-h-screen bg-bg-primary">
      ...
      <section className="relative w-full h-[220px] sm:h-[300px] lg:h-[380px] xl:h-[420px] overflow-hidden">
        <Image src="https://xusywypjmogzbizrwruv.supabase.co/storage/v1/object/public/Images/Page%20superstars/WM2-Ex1.jpg"
          alt="WWE Managers" fill priority sizes="100vw" quality={100} className="object-cover object-[center_30%]"/>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 sm:pb-8 lg:pb-10 px-4">
          <nav className="hidden sm:flex items-center gap-2 text-xs text-text-secondary mb-3">
            ...
            <span className="text-neon-blue">{t('nav.dropdown.managers')}</span>
          </nav>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-6">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1 ml-2">
              {[{k:'name',l:t('superstars.filters.alphabetical')},{k:'manager_matches',l:'🎩 Most Managed'},{k:'manager_wins',l:'✅ Most Wins'},{k:'manager_losses',l:'❌ Most Losses'}].map(s=>(
  ...
  return(
    <Link href={`/superstars/${m.slug}`} className="group relative rounded-2xl border border-border-subtle/20 bg-bg-secondary/20 overflow-hidden transition-all duration-300 hover:border-neon-blue/20 hover:bg-bg-secondary/30 active:scale-[0.98]">
      <div className="relative aspect-square bg-bg-tertiary overflow-hidden">
        {m.photo_url?<Image src={m.photo_url} alt={m.name} fill className="object-cover object-top transition-transform duration-500 group-hover:scale-105" sizes="(max-width:640px)50vw,(max-width:768px)33vw,(max-width:1024px)25vw,20vw"/>
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {m.status==='retired'&&<span className="text-[9px] px-1.5 py-0.5 rounded-md bg-text-secondary/20 border border-text-secondary/20 text-text-secondary font-medium backdrop-blur-sm">{t('common.retired')}</span>}
