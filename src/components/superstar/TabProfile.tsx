    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-border-subtle/30 bg-gradient-to-br from-bg-secondary/20 via-bg-secondary/10 to-transparent">
            <div className="p-5 sm:p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link href="#" onClick={(e) => { e.preventDefault(); document.querySelector<HTMLButtonElement>('[data-tab="matches"]')?.click() }}
          ...
          <div className="relative overflow-hidden rounded-2xl border border-border-subtle/30 bg-gradient-to-br from-bg-secondary/20 via-bg-secondary/10 to-transparent">
            <div className="p-5 sm:p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link href="#" onClick={(e) => { e.preventDefault(); document.querySelector<HTMLButtonElement>('[data-tab="segments"]')?.click() }}
                  className="text-xs text-neon-blue hover:text-neon-blue/80 font-medium flex items-center gap-1 transition-colors">
                  See all <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                </Link>
              </div>
      ...
      <div className="space-y-5">
        ...
        {preview?.statsPreview && (
          <PreviewWidget title="📊 Quick Stats" tabId="statistics" icon="📊">
        ...
        {preview?.finishersPreview?.length > 0 && (
          <PreviewWidget title="💥 Signature Moves" tabId="moves" icon="💥">
            <div className="space-y-2">
        ...
        {preview?.omgPreview?.length > 0 && (
          <PreviewWidget title="⚡ OMG Moments" tabId="omgMoments" icon="⚡">
            <div className="space-y-2">
        ...
        {preview?.tagTeamPreview && (
          <PreviewWidget title="🤝 Tag Team" tabId="tagTeams" icon="🤝">
        ...
        {preview?.stablePreview && (
          <PreviewWidget title="🛡️ Stable" tabId="stables" icon="🛡️">
        ...
        {preview?.mediaPreview?.length > 0 && (
          <PreviewWidget title="📸 Media" tabId="gallery" icon="📸">
            <div className="grid grid-cols-2 gap-2">
...
/* ═══════════════════════════════════════════════════════ */
function PreviewWidget({ title, tabId, icon, children }: { title: string; tabId: string; icon: string; children: React.ReactNode }) {
  const handleSeeMore = (e: React.MouseEvent) => {
    e.preventDefault()
    // Find the tab button by iterating all buttons in the tab bar
    const buttons = document.querySelectorAll('button')
    for (const btn of buttons) {
      if (btn.textContent?.includes(title.replace(/^[^\s]+\s/, '').split(' ')[0])) {
        btn.click()
    ...
    <div className="rounded-2xl border border-border-subtle/30 bg-gradient-to-br from-bg-secondary/20 to-transparent overflow-hidden">
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={handleSeeMore} className="text-[10px] text-neon-blue hover:text-neon-blue/80 font-medium flex items-center gap-0.5 transition-colors">
