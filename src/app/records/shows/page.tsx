  ...
  return (
    <div className="relative">
      <section className="relative w-full h-[220px] sm:h-[300px] lg:h-[380px] overflow-hidden">
        <Image src={HERO} alt="Show & Event Records" fill priority sizes="100vw" quality={100} unoptimized className="object-cover object-center lg:object-[50%_10%]" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 sm:pb-8 lg:pb-10 px-4">
          ...
          <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-text-white text-center tracking-tight mb-2">Show & Event <span className="text-neon-blue">Records</span></h1>
          <p className="text-text-secondary text-sm sm:text-base text-center max-w-2xl">The biggest crowds, the longest cards, and the most prolific shows in WWE history.</p>
        </div>
      </section>
      ...
      <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-6">
        <div className="flex items-center gap-1 overflow-x-auto pb-3 scrollbar-hide border-b border-border-subtle/20 mb-6">
          {TABS.map(t => <button key={t.id} onClick={() => setTab(t.id)} className={`px-4 py-2.5 rounded-xl text-xs font-medium border whitespace-nowrap transition-all ${tab === t.id ? 'bg-neon-blue/15 border-neon-blue/30 text-neon-blue' : 'bg-bg-secondary/30 border-border-subtle/20 text-text-secondary hover:text-text-white'}`}>{t.label}</button>)}
        </div>

        {loading ? <div className="space-y-2">{Array.from({ length: 10 }).map((_, i) => <div key={i} className="h-16 rounded-xl bg-bg-secondary/30 animate-pulse" />)}</div> : list.length === 0 ? <div className="text-center py-20"><p className="text-text-secondary">No records available</p></div> : (
          <div className="mt-2 space-y-1">
            {tab === 'showsByCountry' ? (
              ...
              list.map((item: any, i: number) => (
                <Link key={item.id} href={`/matches/shows/${item.slug}`} className="group flex items-center gap-3 px-4 py-3.5 rounded-lg hover:bg-bg-secondary/40 transition-all border border-transparent hover:border-border-subtle/20">
                  <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${i < 3 ? 'bg-neon-blue/15 text-neon-blue border border-neon-blue/25' : 'bg-bg-tertiary/50 text-text-secondary border border-border-subtle/20'}`}>
                    {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
                  </span>
              ...
              list.map((item: any, i: number) => (
                <Link key={item.id} href={`/shows/${item.slug}`} className="group flex items-center gap-3 px-4 py-3.5 rounded-lg hover:bg-bg-secondary/40 transition-all border border-transparent hover:border-border-subtle/20">
                  <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${i < 3 ? 'bg-neon-blue/15 text-neon-blue border border-neon-blue/25' : 'bg-bg-tertiary/50 text-text-secondary border border-border-subtle/20'}`}>
                    {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
                  </span>
                  {item.series_logo && <div className="w-7 h-7 shrink-0 rounded overflow-hidden"><Image src={item.series_logo} alt="" width={28} height={28} className="w-full h-full object-contain" /></div>}
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-text-secondary">
                      ...
                      {item.city && item.country && <span>{item.city}, {item.country}</span>}
      ...
      <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
        <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/10 p-6 sm:p-8">
          <h2 className="font-display text-xl font-bold text-text-white mb-3">About <span className="text-neon-blue">Show & Event Records</span></h2>
          <p className="text-text-secondary text-sm leading-relaxed">The biggest events in WWE history ranked by attendance, match count, title changes, and global reach. Every stat computed from our complete show database.</p>
        </div>
      </section>
    </div>
  )
