...
const HERO = 'https://xusywypjmogzbizrwruv.supabase.co/storage/v1/object/public/Images/Page%20Hall%20Of%20Fame/603897907_1297244152432362_2277291451456712149_n%20(1).jpg'

export default function YearEndAwardsPage() {
  ...
  const fetchData = useCallback(async (p: number) => {
    ...
    try {
      const r = await fetch(`/api/year-end-list?${params}`)
  ...
  return (
    <div className="relative">
      <section className="relative w-full h-[220px] sm:h-[300px] lg:h-[380px] overflow-hidden">
        <Image src={HERO} alt="WWE Year-End Awards" fill priority sizes="100vw" quality={100} unoptimized className="object-cover object-center" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 sm:pb-8 lg:pb-10 px-4">
          ...
          <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-text-white text-center tracking-tight mb-2">
            <span className="text-neon-blue">Year-End</span> Awards
          </h1>
          <p className="text-text-secondary text-sm sm:text-base text-center max-w-2xl">Annual honors recognizing the top superstars, matches, and storylines of each year.</p>
        </div>
      </section>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {items.map((item: any) => {
                ...
                return (
                  <Link key={item.id} href={href} className="group relative flex flex-col rounded-2xl border border-border-subtle/20 bg-bg-secondary/15 overflow-hidden transition-all hover:border-neon-blue/30 hover:translate-y-[-2px] card-glow">
                    ...
                    <div className="p-3 flex-1 flex flex-col">
                      ...
                      {item.source && <p className="text-[9px] text-text-secondary/60 mt-auto pt-1">{item.source}</p>}
                    </div>
                  </Link>
                )
              })}
      ...
      <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
        <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/10 p-6 sm:p-8">
          <h2 className="font-display text-xl font-bold text-text-white mb-3">About <span className="text-neon-blue">{t('nav.dropdown.yearEndAwards')}</span></h2>
          <p className="text-text-secondary text-sm leading-relaxed">WWE&apos;s annual year-end awards honor the top performers, matches, and storylines of each calendar year. Browse every winner from every year, with full category details and source information.</p>
        </div>
      </section>
    </div>
  )
