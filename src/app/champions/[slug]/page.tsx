...
export default function ChampionshipDetailPage() {
  ...
  const sortedReigns = useMemo(() => {
    let filtered = reigns.filter(r => {
      if (!searchName) return true
      return r.superstar?.name?.toLowerCase().includes(searchName.toLowerCase())
    })
    if (sortBy === 'longest') {
      filtered = [...filtered].sort((a, b) => {
        const ad = a.days_held || (a.lost_date ? 0 : daysSince(a.won_date))
        const bd = b.days_held || (b.lost_date ? 0 : daysSince(b.won_date))
        return bd - ad
      })
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <section className="relative w-full h-[280px] sm:h-[360px] lg:h-[420px] overflow-hidden bg-gradient-to-br from-zinc-900 via-zinc-800 to-black">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 sm:pb-8 lg:pb-10 px-4">
          {champ?.image_url && (
            <div className="relative mb-4">
              <div className="absolute -inset-4 rounded-2xl opacity-30 blur-2xl pointer-events-none bg-neon-blue" />
        ...
        {prevChamp && (
          <Link href={`/champions/${prevChamp.slug}`} className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-bg-primary/80 border border-border-subtle/30 backdrop-blur-sm hover:border-neon-blue/30 transition-all group">
        ...
        {nextChamp && (
          <Link href={`/champions/${nextChamp.slug}`} className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-bg-primary/80 border border-border-subtle/30 backdrop-blur-sm hover:border-neon-blue/30 transition-all group">
        ...
        <div className="flex items-center gap-2">
          {(['history', 'stats'] as TabKey[]).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2.5 rounded-xl text-sm font-medium border whitespace-nowrap transition-all ${activeTab === tab ? 'bg-neon-blue/15 border-neon-blue/30 text-neon-blue' : 'bg-bg-secondary/30 border-border-subtle/20 text-text-secondary hover:text-text-white hover:border-border-subtle/40'}`}>
        ...
        <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
            ...
            <div className="flex items-center gap-1">
              {([
                { key: 'date' as SortKey, label: 'By Date' },
                { key: 'longest' as SortKey, label: 'Longest Reign' },
                { key: 'shortest' as SortKey, label: 'Shortest Reign' },
              ]).map(s => (
                <button key={s.key} onClick={() => setSortBy(s.key)}
                  className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all ${sortBy === s.key ? 'bg-neon-blue/15 border-neon-blue/30 text-neon-blue' : 'bg-bg-secondary/30 border-border-subtle/20 text-text-secondary hover:text-text-white'}`}>
                  {s.label}
                </button>
              ))}
            </div>
                      ...
                      <button onClick={() => setExpandedReign(isOpen ? null : r.id)} className="w-full text-left">
                        ...
                        <div className="hidden lg:grid lg:grid-cols-[60px_minmax(180px,1.5fr)_130px_130px_100px_80px] gap-4 items-center px-5 py-3 hover:bg-bg-secondary/20 transition-all">
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="relative w-10 h-10 rounded-lg overflow-hidden border border-neon-blue/20 shrink-0 bg-bg-tertiary">
                            ...
                            <Link href={`/superstars/${r.superstar?.slug}`} className="text-sm text-text-white font-semibold hover:text-neon-blue transition-colors truncate" onClick={e => e.stopPropagation()}>{r.superstar?.name}</Link>
                          </div>
                        ...
                        <div className="bg-bg-secondary/20 border-t border-border-subtle/10 px-4 sm:px-8 py-4 animate-fade-in">
                          ...
                          <div className="space-y-1">
                            {r.defenses.map((m: any) => (
                              <Link key={m.id} href={`/shows/${m.show?.slug}/matches/${m.slug}`} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-bg-secondary/30 transition-all group">
                                ...
                                {m.is_title_change && <span className="text-[9px] px-1.5 py-0.5 rounded bg-neon-blue/15 border border-neon-blue/25 text-neon-blue font-bold shrink-0">TITLE CHANGE</span>}
              ...
              {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-border-subtle/20">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => goPage(page + 1)} disabled={page === totalPages} className="w-8 h-8 rounded-lg border border-border-subtle/30 flex items-center justify-center text-text-secondary hover:text-text-white disabled:opacity-30 transition-all"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></button>
                  </div>
                </div>
              )}
            </>
          ...
          {statsLoading || !stats ? (
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4">
                {[
                  { label: 'Total Reigns', value: stats.totalReigns, color: 'text-neon-blue' },
                  ...
                  { label: 'Title Matches', value: stats.totalTitleMatches, color: 'text-neon-blue' },
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {stats.longestReign && (
                  <div className="rounded-2xl border border-neon-blue/20 bg-gradient-to-br from-neon-blue/5 to-transparent p-6">
                    <h3 className="text-sm font-bold text-neon-blue uppercase tracking-wider mb-3">Longest Reign</h3>
                    <div className="flex items-center gap-3">
                      <div className="relative w-14 h-14 rounded-xl overflow-hidden border-2 border-neon-blue/30 shrink-0 bg-bg-tertiary">
                      ...
                      <div>
                        <Link href={`/superstars/${stats.longestReign.superstar?.slug}`} className="text-base text-text-white font-bold hover:text-neon-blue transition-colors">{stats.longestReign.superstar?.name}</Link>
                        <p className="text-neon-blue font-bold text-lg">{stats.longestReign.days?.toLocaleString()} days</p>
                ...
                {stats.shortestReign && (
                  <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/15 p-6">
                    <h3 className="text-sm font-bold text-neon-pink uppercase tracking-wider mb-3">Shortest Reign</h3>
                    <div className="flex items-center gap-3">
                      <div className="relative w-14 h-14 rounded-xl overflow-hidden border-2 border-neon-pink/20 shrink-0 bg-bg-tertiary">
                      ...
                      <div>
                        <Link href={`/superstars/${stats.shortestReign.superstar?.slug}`} className="text-base text-text-white font-bold hover:text-neon-pink transition-colors">{stats.shortestReign.superstar?.name}</Link>
                        <p className="text-neon-pink font-bold text-lg">{stats.shortestReign.days?.toLocaleString()} days</p>
              ...
              {stats.mostReigns?.length > 0 && (
                <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/15 p-6">
                  <h3 className="text-sm font-bold text-neon-blue uppercase tracking-wider mb-4">Most Reigns</h3>
                  <div className="space-y-3">
                    {stats.mostReigns.map((m: any, i: number) => (
                      <div key={m.superstar?.id || i} className="flex items-center gap-3">
                        <span className="w-6 h-6 rounded-lg bg-neon-blue/10 flex items-center justify-center text-[10px] text-neon-blue font-bold shrink-0">{i + 1}</span>
                        <div className="relative w-8 h-8 rounded-lg overflow-hidden border border-border-subtle/20 shrink-0 bg-bg-tertiary">{m.superstar?.photo_url ? <Image src={m.superstar.photo_url} alt="" fill className="object-cover object-top" sizes="32px" /> : <div className="w-full h-full flex items-center justify-center text-sm opacity-20">👤</div>}</div>
                        <Link href={`/superstars/${m.superstar?.slug}`} className="text-sm text-text-white font-medium hover:text-neon-blue transition-colors flex-1 truncate">{m.superstar?.name}</Link>
                        <span className="text-sm text-neon-blue font-bold">{m.count}x</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {stats.mostCombinedDays.map((m: any, i: number) => {
                      ...
                      return (
                        <div key={m.superstar?.id || i} className="flex items-center gap-3">
                          <span className="w-6 h-6 rounded-lg bg-neon-blue/10 flex items-center justify-center text-[10px] text-neon-blue font-bold shrink-0">{i + 1}</span>
                          <div className="relative w-8 h-8 rounded-lg overflow-hidden border border-border-subtle/20 shrink-0 bg-bg-tertiary">{m.superstar?.photo_url ? <Image src={m.superstar.photo_url} alt="" fill className="object-cover object-top" sizes="32px" /> : <div className="w-full h-full flex items-center justify-center text-sm opacity-20">👤</div>}</div>
                          <Link href={`/superstars/${m.superstar?.slug}`} className="text-sm text-text-white font-medium hover:text-neon-blue transition-colors w-36 truncate shrink-0">{m.superstar?.name}</Link>
                          <div className="flex-1 h-2 rounded-full bg-bg-tertiary/50 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-neon-blue/60 to-neon-blue transition-all" style={{ width: `${(m.days / maxDays) * 100}%` }} /></div>
              ...
              {stats.byDecade?.length > 0 && (
                <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/15 p-6">
                  <h3 className="text-sm font-bold text-neon-blue uppercase tracking-wider mb-4">Title Changes by Decade</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    {stats.byDecade.map((d: any) => (
                      <div key={d.decade} className="rounded-xl border border-border-subtle/10 bg-bg-tertiary/20 p-3 text-center">
                        ...
                        <span className="block text-lg text-neon-blue font-bold font-display mt-0.5">{d.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section className="max-w-[1440px] mx-auto px-4 sm:px-6 py-8">
        <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/10 p-6 sm:p-8">
          <h2 className="font-display text-xl font-bold text-text-white mb-3">
            <span className="text-neon-blue">{champ?.name || 'Championship'}</span> — Complete Title History
          </h2>
