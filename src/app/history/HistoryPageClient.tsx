...
/* ============================================================
   SCROLL REVEAL — Apple-style component
   ============================================================ */
...
export default function HistoryPageClient() {
  ...
  // Scroll active year into view in timeline
  useEffect(() => {
    if (selectedYear && timelineScrollRef.current) {
      const btn = timelineScrollRef.current.querySelector(`[data-year="${selectedYear}"]`) as HTMLElement
      if (btn) {
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <section className="relative w-full h-[280px] sm:h-[380px] lg:h-[460px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-8 sm:pb-12 lg:pb-16 px-4">
          <motion.h1
            ...
            className="font-display text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-text-white text-center tracking-tight mb-3"
          >
          ...
          >
            From the boxing clubs of Harlem to a global entertainment empire. Every year. Every revolution.
          </motion.p>
        </div>
      </section>

      {/* ===== TIMELINE — Sleek horizontal strip ===== */}
      <section className="sticky top-0 z-40 bg-bg-primary/90 backdrop-blur-xl border-b border-border-subtle/20">
        <div className="max-w-[1440px] mx-auto">
          <div
            ref={timelineScrollRef}
            className="flex items-center overflow-x-auto scrollbar-hide py-3 px-4 sm:px-6 gap-1"
          >
            {loading ? (
              Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="w-16 h-9 rounded-lg bg-bg-secondary/30 animate-pulse shrink-0" />
              ))
            ) : (
              years.map((y) => {
                ...
                return (
                  <button
                    ...
                    className={`relative shrink-0 px-3.5 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-mono font-bold transition-all duration-300 whitespace-nowrap ${
                      isActive
                        ? 'bg-neon-blue text-black shadow-lg shadow-neon-blue/30 scale-105'
                        : 'text-text-secondary hover:text-text-white hover:bg-bg-secondary/40'
                    }`}
                  >
                    {y.year === 1952 ? 'Pre-1953' : y.year}
                    {isActive && (
                      <motion.div
                        layoutId="timeline-indicator"
                        className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-r-[6px] border-t-[6px] border-transparent border-t-neon-blue"
            ...
            >
              {detailLoading ? (
                <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 space-y-8">
                  ...
                  <div className="h-4 w-full rounded bg-bg-secondary/15 animate-pulse" />
                  <div className="h-4 w-3/4 rounded bg-bg-secondary/15 animate-pulse" />
                </div>
              ) : yearDetail?.year ? (
                <>
                  {/* ===== YEAR HERO SECTION ===== */}
                    ...
                    <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
                      <Reveal>
                        <span className="font-display text-7xl sm:text-8xl lg:text-9xl font-bold text-neon-blue/15 select-none">
                          {yearDetail.year.year === 1952 ? 'Origins' : yearDetail.year.year}
                        </span>
                      </Reveal>
                      ...
                      {/* Neon separator */}
                      <Reveal delay={0.3}>
                        <div className="neon-line max-w-xs mx-auto mt-10" />
                      </Reveal>
                    </div>
                  </section>

                  {/* ===== CONTENT BLOCKS — Apple-style scroll reveal ===== */}
                  ...
                  {/* ===== EVENTS FROM wwe_history_events ===== */}
                    ...
                    <section className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
                      <Reveal>
                        <h3 className="font-display text-xl font-bold text-text-white mb-8 flex items-center gap-3">
                          <div className="w-1 h-6 rounded-full bg-neon-blue" />
                          Key Events
                        </h3>
                      </Reveal>
                      ...
                      <Reveal>
                        <div className="rounded-2xl border border-border-subtle/20 bg-bg-secondary/10 p-6 sm:p-8">
                          <h3 className="text-[10px] text-neon-blue uppercase tracking-widest font-bold mb-5">
                            {yearDetail.year.year === 1952 ? 'Pre-1953' : yearDetail.year.year} in Numbers
                          </h3>
                          <div className="flex flex-wrap items-center gap-8 sm:gap-12">
                            {yearDetail.stats.shows > 0 && (
                              <div>
                                <span className="block text-3xl sm:text-4xl font-bold text-text-white font-display">{yearDetail.stats.shows.toLocaleString()}</span>
                                <span className="text-xs text-text-secondary">Shows</span>
                              </div>
                            )}
                            {yearDetail.stats.matches > 0 && (
                              <div>
                                <span className="block text-3xl sm:text-4xl font-bold text-text-white font-display">{yearDetail.stats.matches.toLocaleString()}</span>
                                <span className="text-xs text-text-secondary">Matches</span>
                              </div>
                            )}
                            {yearDetail.stats.titleChanges > 0 && (
                              <div>
                                <span className="block text-3xl sm:text-4xl font-bold text-neon-blue font-display">{yearDetail.stats.titleChanges}</span>
                                <span className="text-xs text-text-secondary">Title Changes</span>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-20">
                      <Reveal>
                        <div className="text-center py-20 rounded-2xl border border-border-subtle/15 bg-bg-secondary/5">
                          ...
                          <p className="text-text-secondary text-base">Content for {yearDetail.year.year === 1952 ? 'Pre-1953' : yearDetail.year.year} is coming soon.</p>
                          <p className="text-text-secondary/40 text-xs mt-2">We&apos;re building the most comprehensive WWE history ever assembled.</p>
                        </div>
                      </Reveal>
                    </div>
                  )}
      ...
      {/* ===== EMPTY STATE — Before any year is selected ===== */}
        ...
        <section className="max-w-3xl mx-auto px-4 sm:px-6 py-20 sm:py-32 text-center">
          ...
          <Reveal delay={0.2}>
            <p className="text-text-secondary text-sm max-w-md mx-auto">
              Select any year from the timeline above to explore the events, stories, and moments that defined that chapter of WWE history.
            </p>
          </Reveal>
        </section>
      )}
...
/* ============================================================
   BLOCK RENDERER — Apple-style content blocks
   ============================================================ */
    ...
    return (
      <Reveal>
        <div className="max-w-3xl mx-auto">
          {block.title && (
            <h3 className="font-display text-xl sm:text-2xl font-bold text-text-white mb-4">{block.title}</h3>
          )}
          <div className="text-text-secondary text-sm sm:text-base leading-[1.85] whitespace-pre-line">
            {block.content_md}
          </div>
        </div>
      </Reveal>
    ...
    return (
      <RevealScale>
        <div className="max-w-3xl mx-auto">
          ...
          {block.image_caption && (
            <p className="text-center text-xs text-text-secondary/50 mt-3 italic">{block.image_caption}</p>
          )}
        </div>
      </RevealScale>
    )
  ...
  if (block.block_type === 'video' && block.video_url) {
    ...
    return (
      <Reveal>
        <div className="max-w-3xl mx-auto">
          {block.title && (
            <h3 className="font-display text-base sm:text-lg font-bold text-text-white mb-4">{block.title}</h3>
          )}
      ...
      <Reveal>
        <div className="max-w-2xl mx-auto py-4">
          <div className="relative text-center">
            ...
            <blockquote className="text-text-white text-base sm:text-lg lg:text-xl italic leading-relaxed font-body relative z-10 pt-4">
              {block.content_md}
            </blockquote>
            {block.title && (
              <p className="text-neon-blue text-sm font-medium mt-4">— {block.title}</p>
            )}
          </div>
        </div>
      </Reveal>
