...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      supabase.from('superstars').select('id, name, slug, photo_url, birth_date').not('birth_date', 'is', null).like('birth_date', `%-${monthStr}-${dayStr}`).limit(20),
    ...
    const birthdays = (birthdayData || []).map(s => ({
      ...s,
