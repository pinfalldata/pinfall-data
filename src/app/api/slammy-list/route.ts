...
export async function GET(request: NextRequest) {
  ...
  try {
    let query = supabase.from('slammy_awards')
    ...
    const { data: allYears } = await supabase.from('slammy_awards').select('year').order('year', { ascending: false })
    const { data: allCats } = await supabase.from('slammy_awards').select('category')
